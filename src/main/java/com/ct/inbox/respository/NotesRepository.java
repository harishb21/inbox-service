...
public interface NotesRepository extends JpaRepository<Notes, Long> {
	...
	@Query(value = "select * FROM notes n INNER JOIN staff_details s on s.user_id = n.receiver_user_id INNER Join roles r on s.role_id = r.role_Id where n.receiver_user_id = :receiver_user_id AND isdeleted=false", nativeQuery = true)
	List<Notes> findAllByReceiver(@Param("receiver_user_id") int receiver_user_id);

	Notes findByNotesid(long notesId);

	@Query(value = "SELECT COUNT(*) FROM notes n INNER JOIN staff_details s on s.user_id = n.receiver_user_id  WHERE n.receiver_user_id = :receiver_user_id AND n.is_read = false",nativeQuery = true)
	long countByIsRead(@Param("receiver_user_id") int receiver_user_id);
